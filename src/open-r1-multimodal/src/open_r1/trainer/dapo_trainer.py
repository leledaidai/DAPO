...
def sync_continue_flag(condition_met: bool):
    flag = torch.tensor([int(condition_met)], device='cuda')
...
##修改epsilon
        ...
        # self.inside_dataloader = self.get_train_dataloader()
        self.inside_dataloader = self.get_sample_dataloader()
        ...
        if self.state.global_step % self.num_iterations == 0:
            ...
            if not self.args.filter_groups_enable:
                ...
                if self.num_gen_batches == 0:
                    ...
                    # print("outside")
                    # for item in inputs:
                    #     print("item['problem']",item['problem'])
                    #     print("item['solution']",item['solution'])
                    generated_data = self._generate_and_score_completions(inputs, model)
                    self.num_gen_batches += 1

                    #log
                    # statement = random.choice([True, False])
                    # print("statement of inputs is ")
                    # print(statement)

                    if (generated_data["std_grouped_rewards"] == 0).any():
                        statement = True
                if sync_continue_flag(statement):
                    max_num_gen_batches = self.args.filter_max_num_gen_batches
                    ...
                    try:
                        ...
                        for _,data in enumerate(self.inside_dataloader):
                            
                            # print("#######用到了inside_dataloader")
                            # print("data为：")
                            # # print(data)
                            # print(self.accelerator.device)
                            # print("inside")
                            ...
                            # #log
                            # statement = random.choice([True, False])
                            # print("statement of data is ", statement)

                            if (generated_data["std_grouped_rewards"] != 0).all():
                                statement = False
                            if not sync_continue_flag(statement):
                            ...
                            if max_num_gen_batches <= 0 or self.num_gen_batches < max_num_gen_batches:
                                ...
                                raise ValueError(
                                    f'self.num_gen_batches={self.num_gen_batches}>=max_num_gen_batches={max_num_gen_batches}. Generated too many. Please check your data.'
                                )
                    except StopIteration:
                        
                        print("inside_dataloader采样完了，重新构建一个新的")
                        self.inside_dataloader = self.get_sample_dataloader()
...
####数据从这里sample
    ...
    def get_sample_dataloader(self) -> DataLoader:
        ...
        train_sampler = self._get_train_sampler(seed)
